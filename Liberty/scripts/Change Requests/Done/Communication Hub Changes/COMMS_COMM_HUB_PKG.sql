...
*
* </pre>         */
procedure execute_broker_statement (pn_broker_id_no in number, pn_company_id_no in number, pv_template_code in varchar2, pv_language_code in varchar2, pd_statement_start_date in date, pd_statement_end_date in date, pv_username in varchar2, pv_return_msg out varchar2);
...
procedure execute_broker_statement (pn_broker_id_no in number, pn_company_id_no in number, pv_template_code in varchar2, pv_language_code in varchar2, pd_statement_start_date in date, pd_statement_end_date in date, pv_username in varchar2, pv_return_msg out varchar2) as
 ...
 cursor c_get_brokers is
   select broker_id_no
     from broker
    where broker_id_no = nvl(pn_broker_id_no,broker_id_no)
...
begin
     ...
     LHHCORR.ws_in_pkg.create_correspondence_prc(p_source => get_system_parameter('LB_HEALTH_COMMS','COMMS_HUB','SOURCE_NAME'),
                                                 ...
                                                 p_source_code => x.broker_id_no,
                                                 p_source_code_type => get_system_parameter('LB_HEALTH_COMMS','COMMS_HUB','BROKER_SRC_CODE'),
                                                 --p_source_version => :p_source_version,
                                                 p_recipient => get_system_parameter('LB_HEALTH_COMMS','COMMS_HUB','BROKER_SRC_CODE')||x.broker_id_no,
       ...
       if lv_external_reference is not null then
         insert into broker_comments (broker_id_no,
                                      comment_datetime,
                                      comment_desc,
                                      last_update_datetime,
                                      username)
                              values (x.broker_id_no,
                                      sysdate,
