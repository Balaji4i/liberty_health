CREATE OR REPLACE EDITIONABLE TRIGGER "LHHCOM"."COMPANY_TABLE_AUDIT_TRG" 
...
BEFORE INSERT OR UPDATE OR DELETE ON COMPANY_TABLE
...
DECLARE
  ...
  aud COMPANY_TABLE_AUDIT%ROWTYPE;
...
BEGIN    
  ...
  ELSIF UPDATING THEN
    ...
    IF :OLD.company_table_id_no <> :NEW.company_table_id_no OR 
      ...
      l_write_aud := 'Y';
    END IF;
    IF :OLD.company_table_desc <> :NEW.company_table_desc OR 
      (:OLD.company_table_desc IS NULL AND :NEW.company_table_desc IS NOT NULL) OR 
      (:OLD.company_table_desc IS NOT NULL AND :NEW.company_table_desc IS NULL) THEN
      aud.transaction_desc := SUBSTR(aud.transaction_desc || ', COMPANY_TABLE_DESC from ' ||
        :OLD.company_table_desc || ' to ' || :NEW.company_table_desc, 1, 550);
      l_write_aud := 'Y';
    END IF;
  ...
  IF INSERTING OR UPDATING THEN
    ...
    aud.company_table_desc               := :NEW.company_table_desc;
  ...
  ELSIF DELETING THEN
    ...
    aud.company_table_desc               := :OLD.company_table_desc;
  ...
  IF l_write_aud = 'Y' THEN
    INSERT INTO COMPANY_TABLE_AUDIT VALUES aud;
  END IF;
