CREATE OR REPLACE EDITIONABLE TRIGGER "LHHCOM"."COMPANY_FEE_TYPE_AUDIT_TRG" 
...
BEFORE INSERT OR UPDATE OR DELETE ON COMPANY_FEE_TYPE
...
DECLARE
  ...
  aud COMPANY_FEE_TYPE_AUDIT%ROWTYPE;
...
BEGIN    
  ...
  IF INSERTING THEN
    aud.transaction_desc := 'Created ' || :NEW.company_fee_type_id_no;
    l_write_aud := 'Y';
  ELSIF UPDATING THEN
    aud.transaction_desc := 'Changed record ' || :OLD.company_fee_type_id_no;
    IF :OLD.company_fee_type_id_no <> :NEW.company_fee_type_id_no OR 
      ...
      l_write_aud := 'Y';
    END IF;
    IF :OLD.company_fee_type_desc <> :NEW.company_fee_type_desc OR 
      (:OLD.company_fee_type_desc IS NULL AND :NEW.company_fee_type_desc IS NOT NULL) OR 
      (:OLD.company_fee_type_desc IS NOT NULL AND :NEW.company_fee_type_desc IS NULL) THEN
      aud.transaction_desc := SUBSTR(aud.transaction_desc || ', COMPANY_FEE_TYPE_DESC from ' ||
        :OLD.company_fee_type_desc || ' to ' || :NEW.company_fee_type_desc, 1, 550);
      l_write_aud := 'Y';
    END IF;
    ...
    END IF;
  ELSIF DELETING THEN
    aud.transaction_desc := 'Deleted record ' || :OLD.company_fee_type_id_no;
    l_write_aud := 'Y';
  END IF;
  ...
  IF INSERTING OR UPDATING THEN
    ...
    aud.company_fee_type_desc            := :NEW.company_fee_type_desc;
  ...
  ELSIF DELETING THEN
    ...
    aud.company_fee_type_desc            := :OLD.company_fee_type_desc;
  ...
  IF l_write_aud = 'Y' THEN
    INSERT INTO COMPANY_FEE_TYPE_AUDIT VALUES aud;
  END IF;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      RAISE;
END COMPANY_FEE_TYPE_AUDIT_TRG;
/