CREATE OR REPLACE EDITIONABLE TRIGGER "LHHCOM"."BROKER_TABLE_TYPE_AUDIT_TRG" 
...
BEFORE INSERT OR UPDATE OR DELETE ON BROKER_TABLE_TYPE
...
DECLARE
  ...
  aud BROKER_TABLE_TYPE_AUDIT%ROWTYPE;
...
BEGIN    
  ...
  IF INSERTING THEN
    aud.transaction_desc := 'Created ' || :NEW.broker_table_type_id_no;
    l_write_aud := 'Y';
  ELSIF UPDATING THEN
    aud.transaction_desc := 'Changed record ' || :OLD.broker_table_type_id_no;
    IF :OLD.broker_table_type_id_no <> :NEW.broker_table_type_id_no OR 
      (:OLD.broker_table_type_id_no IS NULL AND :NEW.broker_table_type_id_no IS NOT NULL) OR 
      (:OLD.broker_table_type_id_no IS NOT NULL AND :NEW.broker_table_type_id_no IS NULL) THEN
      aud.transaction_desc := SUBSTR(aud.transaction_desc || ', BROKER_TABLE_TYPE_ID_NO from ' ||
        :OLD.broker_table_type_id_no || ' to ' || :NEW.broker_table_type_id_no, 1, 550);
      l_write_aud := 'Y';
    END IF;
    IF :OLD.broker_table_type_desc <> :NEW.broker_table_type_desc OR 
      (:OLD.broker_table_type_desc IS NULL AND :NEW.broker_table_type_desc IS NOT NULL) OR 
      (:OLD.broker_table_type_desc IS NOT NULL AND :NEW.broker_table_type_desc IS NULL) THEN
      aud.transaction_desc := SUBSTR(aud.transaction_desc || ', BROKER_TABLE_TYPE_DESC from ' ||
        :OLD.broker_table_type_desc || ' to ' || :NEW.broker_table_type_desc, 1, 550);
      l_write_aud := 'Y';
    END IF;
    ...
    END IF;
  ELSIF DELETING THEN
    aud.transaction_desc := 'Deleted record ' || :OLD.broker_table_type_id_no;
    l_write_aud := 'Y';
  END IF;
  ...
  IF INSERTING OR UPDATING THEN
    aud.broker_table_type_id_no          := :NEW.broker_table_type_id_no;
    aud.broker_table_type_desc           := :NEW.broker_table_type_desc;
  ...
  ELSIF DELETING THEN
    aud.broker_table_type_id_no          := :OLD.broker_table_type_id_no;
    aud.broker_table_type_desc           := :OLD.broker_table_type_desc;
  ...
  IF l_write_aud = 'Y' THEN
    INSERT INTO BROKER_TABLE_TYPE_AUDIT VALUES aud;
  END IF;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      RAISE;
END BROKER_TABLE_TYPE_AUDIT_TRG;
/