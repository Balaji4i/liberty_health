CREATE OR REPLACE EDITIONABLE TRIGGER "LHHCOM"."ACCREDITATION_TYPE_AUD_BEF_TRG"
...
BEFORE INSERT OR UPDATE OR DELETE ON ACCREDITATION_TYPE
...
DECLARE
  ...
  aud ACCREDITATION_TYPE_AUDIT%ROWTYPE;
...
BEGIN    
  ...
  IF INSERTING THEN
    aud.transaction_desc := 'Created ' || :NEW.accreditation_type_id_no;
    l_write_aud := 'Y';
  ELSIF UPDATING THEN
    aud.transaction_desc := 'Changed record ' || :OLD.accreditation_type_id_no;
    IF :OLD.accreditation_type_id_no <> :NEW.accreditation_type_id_no OR
      ...
      l_write_aud := 'Y';
    END IF;
    IF :OLD.accreditation_type_desc <> :NEW.accreditation_type_desc OR
      (:OLD.accreditation_type_desc IS NULL AND :NEW.accreditation_type_desc IS NOT NULL) OR
      (:OLD.accreditation_type_desc IS NOT NULL AND :NEW.accreditation_type_desc IS NULL) THEN
      aud.transaction_desc := SUBSTR(aud.transaction_desc || ', ACCREDITATION_TYPE_DESC from ' ||
        :OLD.accreditation_type_desc || ' to ' || :NEW.accreditation_type_desc, 1, 550);
      l_write_aud := 'Y';
    END IF;
    ...
    END IF;
ELSIF DELETING THEN
    aud.transaction_desc := 'Deleted record ' || :OLD.accreditation_type_id_no;
    l_write_aud := 'Y';
  END IF;
  ...
  IF INSERTING OR UPDATING THEN
    ...
    aud.accreditation_type_desc          := :NEW.accreditation_type_desc;
  ...
  ELSIF DELETING THEN
    ...
    aud.accreditation_type_desc          := :OLD.accreditation_type_desc;
...
-- In Case we don't want to allow deleting
  ...
  IF l_write_aud = 'Y' THEN
    INSERT INTO ACCREDITATION_TYPE_AUDIT VALUES aud;
  END IF;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      RAISE;
END ACCREDITATION_TYPE_AUD_BEF_TRG;
/
