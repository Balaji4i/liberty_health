create or replace procedure cpr_update_grouping_examsc_prc(
    p_input_month_from IN varchar2, 
    p_input_month_to IN VARCHAR2)                            
is

-- This code works for 
-- December 2017, Jan 2018, Febr 2018, March 2018, April 2018, May 2018, June 2018  
-- when using the group EXAMSCOUNCIL and 
-- the rows where finance_receipt_no is 'EXAMSCOUNCIL_20180528' are included 
              ...
              FROM 
                ...
                FROM lhhcom.temp_backup_ls1061b
                   WHERE processed_ind = 'Y' 
                     ...
                     AND group_code = 'EXAMSCOUNCIL'
                     AND FINANCE_RECEIPT_NO = 'EXAMSCOUNCIL_20180528'                     
                     ) cpr_aggr_val            
                GROUP BY   
                  cpr_aggr_val.contribution_date
                , cpr_aggr_val.country_code
...
begin
   ...
   FOR rec IN cpr_fk loop 
    ...
    COMMIT;
end cpr_update_grouping_examsc_prc;   
