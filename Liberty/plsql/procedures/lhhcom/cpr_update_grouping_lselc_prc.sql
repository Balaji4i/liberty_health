create or replace procedure cpr_update_grouping_lselc_prc(
    p_input_month_from IN varchar2, 
    p_input_month_to IN VARCHAR2)                            
is

-- This code works for Jan 2018, Feb 2018, March 2018 and April 2018  
-- when using the group LESOTHOELEC and 
-- the rows where finance_receipt_no is 'LESOTHOELEC_20180428' are included 
              ...
              FROM 
                ...
                FROM lhhcom.temp_backup_ls1061b
                   WHERE processed_ind = 'Y' 
                     ...
                     AND group_code = 'LESOTHOELEC'
                     AND FINANCE_RECEIPT_NO = 'LESOTHOELEC_20180428'                     
                     ) cpr_aggr_val            
                GROUP BY   
                  cpr_aggr_val.contribution_date
                , cpr_aggr_val.country_code
...
begin
   ...
   FOR rec IN cpr_fk loop 
    ...
    COMMIT;
end cpr_update_grouping_lselc_prc;   
