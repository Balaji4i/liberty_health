CREATE OR REPLACE PROCEDURE cpr_update_receipt_no3_prc(p_input_row_choice INTEGER)                            
is

--This procedure is only to balance comms_payments-received with comms_calc_snapshot 
--  where the finance_receipt_no on the tables is THOLOENERGY_20180406 and 
--  the finance_invoice_no on comms_payments_received = 1121 
                ...
                (select
                 ...
                 FROM lhhcom.temp_backup_ls1061b
                   WHERE processed_ind = 'Y'                      
                     AND FINANCE_RECEIPT_NO = 'THOLOENERGY_20180406'
                     and FINANCE_INVOICE_NO = '1121'                                                 
                     ) cpr_aggr_val            
...
begin
   ...
   FOR rec IN cpr_fk loop 
    ...
    COMMIT;
end cpr_update_receipt_no3_prc;                       
