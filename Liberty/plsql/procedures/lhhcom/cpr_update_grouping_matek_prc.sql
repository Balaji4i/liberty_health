create or replace procedure cpr_update_grouping_matek_prc(
    p_input_month_from IN varchar2, 
    p_input_month_to IN VARCHAR2)                            
is

-- This code works for May 2018  
-- when using the group MATEKANE and 
-- the rows where finance_receipt_no is ''MATEKANE_20180502_01' or 'MATEKANE_20180502_02' are included 
              ...
              FROM 
                ...
                FROM lhhcom.temp_backup_ls1061b
                   WHERE processed_ind = 'Y' 
                     ...
                     AND group_code = 'MATEKANE'
                     AND FINANCE_RECEIPT_NO in ('MATEKANE_20180502_01', 'MATEKANE_20180502_02')                     
                     ) cpr_aggr_val            
                GROUP BY   
                  cpr_aggr_val.contribution_date
                , cpr_aggr_val.country_code
...
begin
   ...
   FOR rec IN cpr_fk loop 
    ...
    COMMIT;
end cpr_update_grouping_matek_prc;   
